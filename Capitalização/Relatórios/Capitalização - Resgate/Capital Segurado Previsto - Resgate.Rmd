---
title: "Capital Segurado Previsto - Resgate"
...
resumo_geral <- summary(base_resgate$capitalSeguradoPrevisto)
...
hist(base_resgate$capitalSeguradoPrevisto, breaks = 50, main = "Histograma (sem log)", 
     xlab = "Capital Segurado Previsto", col = "skyblue")

# Histograma com log1p
hist(log1p(base_resgate$capitalSeguradoPrevisto), breaks = 50, main = "Histograma (log(1 + x))", 
...
boxplot(base_resgate$capitalSeguradoPrevisto, 
...
boxplot(log1p(base_resgate$capitalSeguradoPrevisto), 
...
quartis <- quantile(base_resgate$capitalSeguradoPrevisto, probs = c(0, 0.3, 0.7, 1), na.rm = TRUE)

base_resgate$faixaCapitalSeguradoPrevisto <- cut(base_resgate$capitalSeguradoPrevisto,
                                         breaks = quartis,
                                         labels = c("Baixo", "Medio", "Alto"),
                                         include.lowest = TRUE)

resumo_por_faixa <- base_resgate |>
...
# Gráficos
km = survfit2(Surv(tempo_mes, delta) ~ faixaCapitalSeguradoPrevisto, data = base_resgate)

km_surv <- tidy_survfit(km, type = "survival")

p = ggplot(base_resgate) + theme_classic()
